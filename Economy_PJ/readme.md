...
1. Python을 활용한 Server
	1.  기본적인 API요청 등은 Python을 활용
	2.  받아온 데이터를 가공하는 과정에서 연산의 최적화를 위해 부분적으로 C를 사용
	3.  최종적으로 RUST로 언어를 바꾸는 것이 목적
2. JS, HTML, CSS를 활용한 Client
	1.  '한국'의 환율 그래프(1개)를 표시
	2.  '한국'의 경제지표 그래프 2개를 표시
		1. 경제지표 선택지는 하드코딩
	3. '한국'의 경제지표 그래프 N개를 선택해서 표시
		1. 경제지표 선택지는 하드코딩
		2. 경제지표 선택지를 API로 선택할 수 있게 수정
	4. '임의'의 국가의 경제지표 그래프를 N개 선택가능
